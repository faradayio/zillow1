class StatisticalArea < ActiveRecord::Base
  ...
  class << self
    def days
      connection.select_values('SELECT DISTINCT DATE(appearances.appeared_at) AS d FROM appearances WHERE appearances.appeared_at IS NOT NULL ORDER BY d LIMIT 5').map { |raw| Date.parse raw }
    end
    cache_method :days
